...
import { ITemplateConfigDTO } from '../dtos/ITemplateConfigDTO';
import { ILanguageConfigDTO } from '../dtos/ILanguageConfigDTO';
import { ParseContentService } from './ParseContentService';

export class RegisterTemplateService {
  readonly #languageConfig: Record<
    ITemplateConfigDTO['language'],
    ILanguageConfigDTO
  >;

  readonly #parseContentService: ParseContentService;
  ...
  #formatPeriod(
    ...
    if (item.currently || !item.endsAt) {
      return `${startDate} - ${
        this.#languageConfig[templateConfig.language].period.untilNow
      }`;
    }
  ...
  #joinKeywords(keywords: Array<string>): string {
    return keywords.join(',');
  }

  #prepareItems(
    templateConfig: ITemplateConfigDTO,
    data: Array<IPeriodItemDTO>,
  ): Array<Omit<IPeriodItemDTO, 'keywords'> & { keywords: string }> {
    if (!data?.length) {
      return [];
    }
